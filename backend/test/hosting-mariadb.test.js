// Test de integración REAL contra MariaDB para el dominio hosting_plans +
// hosting_services (Fase DB-3C), mismo patrón que clients-mariadb.test.js:
// ejercita el flujo HTTP completo con el driver mysql2 real.
//
...
const FIXTURE = path.join(__dirname, "fixtures", "mariadb-hosting-flow.mjs");
...
// consulta authRequired), clients (FK de hosting_services), hosting_plans y
// hosting_services (dominio bajo prueba). Mantener en sync manualmente con
...
test(
  "hosting_plans/hosting_services: flujo completo contra MariaDB real (plans CRUD, services CRUD, cambio de plan, suspender/reactivar, FKs)",
  ...
  async (t) => {
    ...
    assert.match(result.stdout, /MARIADB_HOSTING_FLOW_OK/);
  },
);
