...
Línea base esperada de un checkout nuevo: **172 tests, 169 pass / 3 fail / 0 skip contra MariaDB** (con `MARIADB_TEST_URL`); **160 pass / 3 fail / 9 skip contra Postgres** (sin ella, los 9 tests de integración MariaDB se saltean).

## Prueba de integración real contra MariaDB (dominios auth/users, clients, hosting_plans/hosting_services, audit_logs, domains, Support/Tickets, Tasks, Settings, Infrastructure Services)

`auth-mariadb.test.js`, `clients-mariadb.test.js`, `hosting-mariadb.test.js`, `audit-mariadb.test.js`, `domains-mariadb.test.js`, `support-mariadb.test.js`, `tasks-mariadb.test.js`, `settings-mariadb.test.js` e `infra-services-mariadb.test.js` son las pruebas de la suite que hablan con un motor MariaDB real en vez de mockear `pool.query`/`pool.connect` — necesarias porque un mock no puede confirmar que las queries convertidas (placeholders `?`, el patrón UPDATE+SELECT que reemplaza a `RETURNING`, la normalización de errores de duplicado, `LOWER()/LIKE` en vez de `ILIKE`, `COUNT(CASE WHEN...)` en vez de `FILTER`, la columna reservada `` `key` ``, fechas calculadas en Node en vez de `DATE_TRUNC`/`INTERVAL`) realmente funcionan contra el driver `mysql2`.

- **Sin `MARIADB_TEST_URL` seteada**: los nueve tests se saltean (`skip`), no fallan. Es el caso normal en un entorno sin MariaDB disponible.
...
- **MariaDB**: `auth-mariadb.test.js`, `clients-mariadb.test.js`, `hosting-mariadb.test.js`, `audit-mariadb.test.js`, `domains-mariadb.test.js`, `support-mariadb.test.js`, `tasks-mariadb.test.js`, `settings-mariadb.test.js` e `infra-services-mariadb.test.js` son la excepción real (no mock) — ver la sección dedicada más abajo.
...
- La cobertura de código no se mide (no hay `c8`/`nyc` configurado). Los tests actuales (172, contando las nueve pruebas de integración MariaDB y la de protección de fixtures) apuntan a los hallazgos de seguridad encontrados y a los endpoints más sensibles, no a cobertura exhaustiva de todos los controllers/servicios.
