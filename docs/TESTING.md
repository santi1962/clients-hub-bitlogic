...
- El fixture (`fixtures/mariadb-auth-flow.mjs`) tiene un guard explícito (`MARIADB_FIXTURE_RUN=1`, seteado solo por el propio test) — ver la sección de protección de fixtures abajo.
...
- La cobertura de código no se mide (no hay `c8`/`nyc` configurado). Los tests actuales (68, contando la prueba de integración MariaDB y la de protección de fixtures) apuntan a los hallazgos de seguridad encontrados y a los endpoints más sensibles, no a cobertura exhaustiva de todos los controllers/servicios.
