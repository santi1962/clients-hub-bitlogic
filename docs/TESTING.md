...
Ambos ejecutan `node --test "test/**/*.test.js"` (glob explícito desde la Fase DB-2.5, ver más abajo por qué), el test runner nativo de Node (no se instaló Jest, Vitest, Mocha ni ningún otro framework). Requiere Node `>=22.12.0` (ver `engines` en `package.json`, reforzado por `.npmrc` con `engine-strict=true` y por el chequeo en runtime de `backend/src/utils/assert-node-version.js` — ver `test/node-version.test.js` para la prueba unitaria de esa lógica). No hace falta ninguna variable de entorno especial para la suite normal: los tests no dependen de credenciales reales de ninguna integración externa. La única excepción es `MARIADB_TEST_URL` (opcional, ver más abajo), usada por las pruebas de integración `*-mariadb.test.js` de cada dominio funcional.
...
**MariaDB es el único motor soportado** (desde la Fase DB-5A no queda ningún código Postgres en runtime). Para levantar una instancia descartable donde correr `MARIADB_TEST_URL=mysql://... npm test`, usar `npm run db:schema:mariadb -- --url mysql://... --confirm-production` (o cualquier MariaDB 11.4 local/Docker) — ver `docs/MARIADB_MIGRATION.md` para el detalle de cómo se levantó durante la migración.
...
└── fixtures/
    ...
    └── mariadb-full-app-smoke.mjs       # Corrido en un proceso hijo por full-app-mariadb-smoke.test.js — a diferencia de los demás, arranca src/server.js como un proceso de SO aparte (no importa app.js en memoria), para probar el arranque/apagado real
```

## Línea base de tests (actualizado en la Fase DB-5A)

Desde el cierre de la Fase DB-5A (MariaDB único motor, `pg` removido) la suite corre verde contra una MariaDB 11.4 descartable con `MARIADB_TEST_URL` seteada. Los 3 fallos históricos que existían mientras convivían Postgres y MariaDB (2 en `health.test.js`, 1 en `settings-plans-authorization.test.js`, todos por falta de un Postgres real configurado en `backend/.env`) dejaron de aplicar — no queda ningún camino de test que dependa de un Postgres real, porque no queda ningún Postgres. Ver `docs/MARIADB_MIGRATION.md` para el historial de líneas base intermedias durante la migración (útil solo como referencia histórica).

Línea base tras el cierre pre-deploy (Node `>=22.12.0` + fix de `updateCompanyLogo`): **206/206 verde, 0 skipped**, contra MariaDB real con schema aplicado. Suma 5 tests de `assert-node-version` (política de Node) y 3 de la corrección del bug de logo (2 unitarios en `settings-domain.test.js`, 1 a nivel HTTP en `settings-plans-authorization.test.js`, más el caso agregado al fixture real de `mariadb-settings-flow.mjs`).

Sin `MARIADB_TEST_URL` seteada, los tests de integración `*-mariadb.test.js` se saltean (`skip`) — el resto de la suite (mockeada) sigue corriendo igual.

## Prueba de integración real contra MariaDB (todos los dominios funcionales + aplicación completa)

`auth-mariadb.test.js`, `clients-mariadb.test.js`, `hosting-mariadb.test.js`, `audit-mariadb.test.js`, `domains-mariadb.test.js`, `support-mariadb.test.js`, `tasks-mariadb.test.js`, `settings-mariadb.test.js`, `infra-services-mariadb.test.js`, `billing-mariadb.test.js` y `full-app-mariadb-smoke.test.js` son las pruebas de la suite que hablan con un motor MariaDB real en vez de mockear `pool.query`/`pool.connect` — necesarias porque un mock no puede confirmar que las queries convertidas (placeholders `?`, el patrón UPDATE+SELECT que reemplaza a `RETURNING`, la normalización de errores de duplicado, `LOWER()/LIKE` en vez de `ILIKE`, `COUNT(CASE WHEN...)` en vez de `FILTER`, la columna reservada `` `key` ``, fechas calculadas en Node en vez de `DATE_TRUNC`/`INTERVAL`, `NEXTVAL`/`SETVAL` con branching por driver) realmente funcionan contra el driver `mysql2`.

- **Sin `MARIADB_TEST_URL` seteada**: los once tests se saltean (`skip`), no fallan. Es el caso normal en un entorno sin MariaDB disponible.
...
- `full-app-mariadb-smoke.test.js` (Fase DB-3K) es distinto a todos los anteriores: en vez de importar `app.js` en el mismo proceso (`startEphemeralServer`), arranca `src/server.js` como un **proceso de sistema operativo real** (`SCHEDULER_ENABLED=false`, integraciones externas deshabilitadas) y confirma, contra MariaDB real: la conexión (`SELECT 1`) antes de levantar el HTTP server, `health/live`/`health/ready`, login + refresh reales (JWT + cookie httpOnly), las familias de rutas principales (usuarios, clientes, planes, hosting, dominios, tickets, tareas, settings, templates, automation settings, scheduler de solo lectura, dashboard + `analyticsData`, avisos/pagos/cobranza, portal cliente), un backup real terminando en `success` (`mariadb-dump`/`mysqldump`), el handshake de Engine.IO (confirma que Socket.IO se inicializó, sin agregar `socket.io-client` como dependencia), la política de uploads, y que el proceso efectivamente termina tras `SIGTERM` (en Windows no se puede confirmar el camino completo de apagado ordenado paso a paso — ver nota de plataforma en `docs/MARIADB_MIGRATION.md`).

## Test de guardia: cero PostgreSQL en runtime (endurecido en la Fase DB-5A)

`no-postgres-runtime-sql.test.js`, originado en la Fase DB-3K como un escaneo de sintaxis SQL Postgres-only, se endureció en la Fase DB-5A para verificar que PostgreSQL desapareció del todo, no solo su sintaxis SQL. Ahora chequea:

- **Sin import de `pg` en ningún archivo de runtime** — incluidos `db/pool.js` y `db/reset.js`, que antes estaban explícitamente excluidos del escaneo (ya no hace falta excluirlos: no tienen ninguna rama de código Postgres que preservar).
- **Sin `pg` en las `dependencies` de `package.json`** — el paquete fue desinstalado del todo.
- **Sin ninguna línea activa (no comentada) `postgresql://`** en `backend/.env.example` ni `backend/.env.production.example`.
- **Sin sintaxis SQL Postgres-only**: placeholders `$N`, `RETURNING`, `ILIKE`, `FILTER (WHERE...)`, `DATE_TRUNC`, `generate_series`, casts `::tipo`, `INTERVAL '...'` literal, `ANY(...)`, `ON CONFLICT` — todo `backend/src/{services,controllers,routes,jobs,middlewares}/**` más `app.js`/`server.js`/`socket.js`.
- **Sin `config.db.driver`** en ningún lado del repo — ese concepto de branching dual-driver está retirado del todo, no queda ninguna decisión en runtime basada en qué motor está activo (solo hay uno).

Excepciones explícitas (no escaneadas): `scripts/**`/`seeds/**` (no son runtime — ver la clasificación de seeds/scripts en `docs/MARIADB_MIGRATION.md`), y `db/archive/postgresql-migrations/*.sql` (migraciones históricas de Postgres, archivadas a propósito, nunca ejecutables).

Este test **no es cosmético**: durante su desarrollo detectó `emailService.listLogs()` (`ILIKE`+`$N`+`COUNT(*)` sin alias) sin que ninguna fase anterior lo hubiera visto, y se verificó activamente que funciona inyectando un `$1` de prueba en un archivo ya convertido (falló como se esperaba, se revirtió).
...
- No hay pruebas de carga/performance ni de concurrencia real de MariaDB (el pool se mockea en los tests que lo necesitan).
- La cobertura de código no se mide (no hay `c8`/`nyc` configurado). Los tests actuales (206, contando las pruebas de integración MariaDB, el test de guardia de runtime, el de política de Node y la de protección de fixtures) apuntan a los hallazgos de seguridad encontrados y a los endpoints más sensibles, no a cobertura exhaustiva de todos los controllers/servicios.
