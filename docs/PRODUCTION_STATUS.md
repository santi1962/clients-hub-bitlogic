...
7. ~~A verificar (hallazgo de la Fase DB-3H): gap de `company_settings.logo_url` contra el Postgres real del VPS~~ — **ya no aplica desde la Fase DB-5A**, mismo motivo que el punto anterior. ~~Bug de `updateCompanyLogo` reventando por `company_name` (`NOT NULL`) al subir un logo antes de guardar la empresa~~ — **corregido en el cierre pre-deploy**: si no existe todavía `company_settings`, el endpoint responde `409` con un mensaje funcional claro (`"Todavía no se guardó la configuración de la empresa..."`) en vez de un `500` genérico, y no se inserta ninguna fila incompleta. Con una fila ya existente, el comportamiento no cambió (solo actualiza `logo_url`). Ver `settings-domain.test.js`, `settings-plans-authorization.test.js` y el fixture real `mariadb-settings-flow.mjs`.
...
12. **MariaDB es el único motor soportado desde el cierre de la Fase DB-5A**: `pg` fue removido de `package.json`, `db/pool.js` usa solo `mysql2/promise`, y `config/index.js` rechaza `postgresql://` al arrancar con un error fatal claro. La suite de tests corre verde contra una MariaDB 11.4 descartable, 0 skipped (206/206 tras el cierre pre-deploy). **Esto sigue siendo trabajo de código en la rama `migration/mariadb-only-v1`, no mergeado a `main` ni desplegado**: el VPS no fue tocado y no existe todavía ninguna base de datos de producción — cuando se despliegue, se crea fresca (Hestia/phpMyAdmin), sin migrar datos de ningún Postgres anterior.
13. **Node `>=22.12.0` es el requisito real del proyecto (cierre pre-deploy)**: antes, `engines.node` en ambos `package.json` (frontend y backend) decía `>=18.0.0`, desalineado con lo que `@tanstack/react-start` exige de verdad. Ahora una sola política en todo el repo, reforzada con `.npmrc` (`engine-strict=true`) y un chequeo en runtime (`backend/src/utils/assert-node-version.js`, primera línea de `server.js`) que corta el arranque con un mensaje claro si el Node real no cumple. Verificado: `npm ci` limpio, build SSR del frontend, suite completa y smoke test, todo corrido sobre Node 22.19.0.
