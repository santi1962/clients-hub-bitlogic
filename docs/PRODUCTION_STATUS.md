...
12. **Código de aplicación 100% ejecutable contra MariaDB (Fase DB-3K, última fase funcional de la migración)**: con esto, todo `backend/src` (todos los dominios de negocio + Backups + Email/Logs + startup/health) tiene sus queries convertidas y probadas contra MariaDB real, confirmado por un test de guardia automatizado (escanea el repo entero) y un smoke test que arranca el backend completo contra MariaDB real. **No obstante, la aplicación NO puede activarse sobre MariaDB en producción todavía**: falta (a) migrar los datos reales (próxima fase, deliberadamente fuera de alcance de DB-3K — no se tocó ningún dato real ni el VPS), y (b) una ventana de validación con datos reales contra ambos motores en paralelo (ninguna fase de esta migración pudo validarse contra el Postgres real de producción, sin credenciales en ningún entorno de trabajo usado).
