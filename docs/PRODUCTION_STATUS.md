...
- **Backend:** Express + Node ESM, PostgreSQL con SQL directo (sin ORM). **Migración a MariaDB — código de aplicación completo** (mergeada a `main`, el histórico `migration/mariadb` quedó contenido enteramente en el merge `70fec99`): el motor productivo definitivo pasa a ser MariaDB (VPS real: MariaDB 11.4.10, `utf8mb4`/`utf8mb4_unicode_520_ci`), pero hoy PostgreSQL sigue siendo el único motor activo en producción. `backend/db/schema.sql` ya está normalizado a esa collation en las 20 tablas y validado contra MariaDB 11.4 real; **todo** el código de runtime (services/controllers/routes/jobs/middlewares, incluidos Backups y los módulos de Email/Logs) ya está convertido para funcionar contra ambos motores, verificado con un test de guardia automatizado que escanea el repo entero en busca de sintaxis Postgres-only y con un smoke test que arranca el backend completo contra MariaDB real. Ver `docs/MARIADB_MIGRATION.md`. **Aun así, no cambiar `DATABASE_URL` a `mysql://` en ningún ambiente real todavía**: falta migrar los datos reales (próxima fase) y validar contra el Postgres real de producción.
...
10. **Bug preexistente corregido en la Fase DB-3K, no introducido por esta migración**: `onboarding.routes.js` consultaba una tabla `billing_notices` que **nunca existió** en este proyecto (la tabla real es `payment_notices`) — `GET /api/onboarding/` daba 500 en producción real, contra Postgres, desde antes de esta migración. Ya corregido.
11. **Backups convertido en la Fase DB-3K, corrigió de paso un bug real**: el módulo Backups (antes 100% `pg_dump`) tenía además un `INSERT` con 6 columnas pero solo 5 valores en el `VALUES` — no hubiera funcionado en **ningún** motor. Ya corregido y probado contra MariaDB real (dump real con `mariadb-dump`/`mysqldump`).
12. **Código de aplicación 100% ejecutable contra MariaDB (Fase DB-3K)**: todo `backend/src` (todos los dominios de negocio + Backups + Email/Logs + startup/health) tiene sus queries convertidas y probadas contra MariaDB real, confirmado por un test de guardia automatizado (escanea el repo entero) y un smoke test que arranca el backend completo contra MariaDB real.
13. **Toolkit de migración de datos reales construido y validado con datos sintéticos, pendiente de ejecución real (Fase DB-4A, rama `migration/mariadb-real-data-v1`)**: `backend/scripts/mariadb-migration/` (auditoría, backup, exportación NDJSON, importación con orden FK y transacciones, validación en 17 dimensiones, pruebas funcionales) probado de punta a punta contra un par de bases sintéticas (Postgres + MariaDB descartables) — sin diferencias, 17/17 categorías OK. **No se ejecutó ni una vez contra el PostgreSQL real** (sin credenciales en este entorno — el usuario no recuerda la contraseña del Postgres local, no se intentó forzar). **La aplicación NO puede activarse sobre MariaDB en producción todavía**: falta (a) conseguir credenciales y correr el toolkit contra los datos reales, (b) una ventana de validación con datos reales contra ambos motores en paralelo, y (c) mergear `migration/mariadb-real-data-v1` a `main` (no se hizo en esta fase). Ver `docs/MARIADB_DATA_MIGRATION.md` para el procedimiento completo.
