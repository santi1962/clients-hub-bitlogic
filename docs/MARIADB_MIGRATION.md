...
**No cambiar `DATABASE_URL` a `mysql://` en ningún ambiente real todavía.** Los dominios auth/users, **clients**, **hosting_plans/hosting_services**, **`audit_logs`**, **domains**, **Support/Tickets**, **Tasks**, **Settings** (`company_settings`) y **Infrastructure Services** (`email_templates`, `automation_settings`, `scheduler_logs`, `dashboard.service.js`) tienen sus queries convertidas (ver más abajo) — solo queda **facturación** (`billing.service.js`/`.controller.js`, `payments`, `payment_notices`, MercadoPago) y **deploy/VPS**, que siguen escritos en sintaxis PostgreSQL y romperían contra MariaDB.
...
- **Última fase cerrada:** DB-3I (**Infrastructure Services**: Email Templates + Automation Settings + Scheduler + Dashboard, incluyendo las sub-queries de solo-lectura de `payments`/`payment_notices` dentro de `dashboard.service.js`, por decisión explícita del usuario — sin tocar `billing.service.js`/controller/routes).
- **Qué está hecho:** capa de compatibilidad dual-driver (`pg`/`mysql2`) en `backend/src/db/pool.js`, schema MariaDB consolidado y con collation normalizada (`backend/db/schema.sql`, validado contra MariaDB 11.4 real), y los dominios **auth/users**, **clients**, **hosting_plans/hosting_services**, **`audit_logs`**, **domains**, **Support/Tickets**, **Tasks**, **Settings** e **Infrastructure Services** (`email_templates`, `automation_settings`, `scheduler_logs`, `dashboard.service.js`) con sus queries convertidas y probadas contra ambos motores. El punto ciego de auditoría contra MariaDB (documentado en DB-3B/DB-3C) ya no existe para ninguno de estos.
- **Qué falta:** solo **facturación/`billing`** (es el módulo con más incompatibilidades: `UPDATE...RETURNING`, `FILTER`, `generate_series`, MercadoPago) y, al final, deploy/VPS/push.
...
**Línea base actual (tras DB-3I):**

| Motor | tests | pass | fail | skip |
|---|---|---|---|---|
| Postgres | 172 | 160 | **3** | 9 |
| MariaDB | 172 | 169 | **3** | 0 |
...
| Facturación (`billing`) | Último módulo pendiente — el de más incompatibilidades (`UPDATE...RETURNING`, `FILTER`, `generate_series`, MercadoPago) | ⏳ pendiente |
...
| `automation_settings` | **Sí, a propósito** — el seed `INSERT IGNORE` de 8 defaults del propio `db/schema.sql` no envía id explícito (confirmado en DB-3I) | Actualizar ese seed para mandar id explícito | Sin fecha — no bloquea producción (el `INSERT IGNORE` corre como parte del schema, no un seed de datos demo) |
...
| `email_logs` | Sí | Módulo Email/notificaciones | A definir (fuera del alcance de Infrastructure Services) |
...
- **Cross-dependencias de `company_settings`/`automation_settings` con otros módulos**: `email.service.js`/`hestia.service.js` no las tocan (leen de `config`/env, no de estas tablas). Los jobs de scheduler (`payment-reminders.job.js`, `hestia-sync.job.js`, `delinquency-detection.job.js`, wireados por `scheduler-init.service.js`) llaman a `automationSettingsService`/`schedulerService` (ya convertidos), pero sus propios cuerpos de job pueden tener queries adicionales contra otras tablas todavía no auditadas en esta fase — no se revisaron esos archivos, fuera del alcance explícito de DB-3I (solo "wiring" de jobs registrados, no su lógica interna).
- **`dashboard.service.js` ya convertido incluye sub-queries de solo-lectura contra `payments`/`payment_notices`** (decisión explícita del usuario en DB-3I) — pero `billing.service.js`/`.controller.js`/rutas, que son los dueños reales de esas tablas (dan de alta/modifican pagos y avisos), siguen 100% en sintaxis Postgres. Cambiar `DATABASE_URL` a `mysql://` seguiría rompiendo el módulo de facturación aunque el dashboard ya no rompa.
- **`app.js` healthcheck (`/api/system/status`) y `dashboard.service.js` leen `clients`/`hosting_services`/`domains`/`support_tickets`/`internal_tasks` vía `COUNT(*) as count`** — ya convertidas en fases previas, sin cambios nuevos en DB-3I más allá de la query de `scheduler_logs` en `app.js`.
