...
**No cambiar `DATABASE_URL` a `mysql://` en ningún ambiente real todavía.** Los dominios auth/users, **clients**, **hosting_plans/hosting_services**, **`audit_logs`**, **domains**, **Support/Tickets** y **Tasks** tienen sus queries convertidas (ver más abajo) — el resto de los módulos (facturación, configuración, backups, scheduler) siguen escritos en sintaxis PostgreSQL y romperían contra MariaDB.
...
- **Última fase cerrada:** DB-3G (dominio **Tasks** convertido: `internal_tasks`).
- **Qué está hecho:** capa de compatibilidad dual-driver (`pg`/`mysql2`) en `backend/src/db/pool.js`, schema MariaDB consolidado y con collation normalizada (`backend/db/schema.sql`, validado contra MariaDB 11.4 real), y los dominios **auth/users**, **clients**, **hosting_plans/hosting_services**, **`audit_logs`**, **domains**, **Support/Tickets** y **Tasks** con sus queries convertidas y probadas contra ambos motores. El punto ciego de auditoría contra MariaDB (documentado en DB-3B/DB-3C) ya no existe para ninguno de estos.
- **Qué falta:** todo lo demás (Fase DB-3H en adelante) — convertir el resto de los módulos uno por uno. Con `clients`/`hosting_services`/`domains`/`support_tickets`/`internal_tasks` ya convertidos, `settings` es un candidato razonable, dejando facturación/`billing` para el final (es el módulo con más incompatibilidades: `UPDATE...RETURNING`, `FILTER`, `generate_series`).
...
**Línea base actual (tras DB-3G):**

| Motor | tests | pass | fail | skip |
|---|---|---|---|---|
| Postgres | 148 | 138 | **3** | 7 |
| MariaDB | 148 | 145 | **3** | 0 |
...
| DB-3H en adelante | Resto de los módulos (facturación, configuración, backups, scheduler) | ⏳ pendiente |
