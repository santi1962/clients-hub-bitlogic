...
**No cambiar `DATABASE_URL` a `mysql://` en ningún ambiente real todavía.** Los dominios auth/users, **clients**, **hosting_plans/hosting_services**, **`audit_logs`** y **domains** tienen sus queries convertidas (ver más abajo) — el resto de los módulos (facturación, soporte, tareas, configuración, backups, scheduler) siguen escritos en sintaxis PostgreSQL y romperían contra MariaDB.
...
- **Última fase cerrada:** DB-3E (dominio **domains** convertido).
- **Qué está hecho:** capa de compatibilidad dual-driver (`pg`/`mysql2`) en `backend/src/db/pool.js`, schema MariaDB consolidado y con collation normalizada (`backend/db/schema.sql`, validado contra MariaDB 11.4 real), y los dominios **auth/users**, **clients**, **hosting_plans/hosting_services**, **`audit_logs`** y **domains** con sus queries convertidas y probadas contra ambos motores. El punto ciego de auditoría contra MariaDB (documentado en DB-3B/DB-3C) ya no existe para ninguno de estos.
- **Qué falta:** todo lo demás (Fase DB-3F en adelante) — convertir el resto de los módulos uno por uno. `support`/`tasks` son candidatos razonables (dependen de `clients`/`hosting_services`/`domains`, ya convertidos), dejando facturación/`billing` para el final (es el módulo con más incompatibilidades: `UPDATE...RETURNING`, `FILTER`, `generate_series`).
...
**Línea base actual (tras DB-3E):**

| Motor | tests | pass | fail | skip |
|---|---|---|---|---|
| Postgres | 123 | 115 | **3** | 5 |
| MariaDB | 123 | 120 | **3** | 0 |
...
| DB-3F en adelante | Resto de los módulos (facturación, soporte, tareas, configuración, backups, scheduler) | ⏳ pendiente |
